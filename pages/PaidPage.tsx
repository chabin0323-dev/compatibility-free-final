...
const PaidPage: React.FC = () => {
  const handlePayment = () => {
    window.location.href = 'https://www.paypal.com/ncp/payment/AMEJ4V5C564UN';
  };

  const openInBrowser = () => {
    window.location.href = 'https://nexa-lovelab.com/paid?ref=browser';
  };
  ...
  return (
    <div style={{ backgroundColor: '#0f021b', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif' }}>
      <div style={{ maxWidth: '420px', width: '100%', border: '1px solid rgba(180,100,255,0.4)', borderRadius: '20px', padding: '32px 24px', backgroundColor: 'rgba(30,10,50,0.9)' }}>
        ...
        {/* ブラウザで開くボタン */}
        <div style={{ background: 'linear-gradient(90deg, #ff6b35, #f7c948)', borderRadius: '50px', padding: '2px', marginBottom: '16px' }}>
          <button
            onClick={openInBrowser}
            style={{ width: '100%', padding: '16px', borderRadius: '50px', background: '#1a0035', border: 'none', cursor: 'pointer', fontSize: '15px', fontWeight: 'bold', color: '#ffffff' }}
          >
            📱 うまく開けない方はこちら<br />
            <span style={{ fontSize: '12px', color: '#f7c948' }}>→ ブラウザで開いて決済する</span>
          </button>
        </div>
